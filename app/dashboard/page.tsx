...
export default function DashboardPage() {
  ...
  const handleReserve = async (slot: ParkingSlot) => {
    ...
    const patch = { status: "reserved" as const, reservedBy: user.email, reservedAt: Date.now() }
  ...
  const handleBollardToggle = async (slot: ParkingSlot) => {
    ...
    syncSlots(updated)
    setSelectedSlot(updated.find((s) => s.id === slot.id) || null)
    // Use dedicated bollard endpoint so ESP32 can rely on it
    try {
      await fetch("/api/bollard", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ slotId: slot.id, bollardUp: !slot.bollardUp }),
      })
    } catch { /* offline */ }
  }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {[
                  ...
                  },
                  {
                    ...
                    desc: "Once your vehicle enters, the parking sensor detects it and automatically changes the slot status from Reserved to Occupied (red). Press Simulate Car Detected to demo this in the prototype.",
                  },
                  {
                    step: "6",
                    ...
                    desc: "When you leave, press Simulate Car Left on the occupied slot card or inside the slot details. The slot resets to Available so the next driver can reserve it.",
                  },
            ...
            >
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {slot.status === "occupied" && (
                  <button
                    onClick={() => handleCarLeft(slot)}
                    className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-medium transition-colors"
                  >
              ...
              <div className="space-y-4">
                ...
                {selectedSlot.paid && selectedSlot.activeQrToken && (
                  <div className="space-y-3">
                      ...
                      {selectedSlot.status === "occupied" && (
                        <div className="mt-2 space-y-2">
                          <p className="text-sm text-green-400 flex items-center gap-1.5">
                            <ShieldCheck className="w-4 h-4" /> Car detected — slot is now Occupied
                          </p>
                          <button
                            onClick={() => handleCarLeft(selectedSlot)}
                            className="w-full flex items-center justify-center gap-2 py-2.5 bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-semibold text-sm transition-colors"
                          >
                            <Car className="w-4 h-4" /> Simulate Car Left — Free Slot
                          </button>
                        </div>
                      )}
                    </div>
                    ...
                    {/* Sensor Simulation — only when reserved+paid, not yet occupied */}
                    {selectedSlot.status === "reserved" && selectedSlot.reservedBy === user.email && (
                      <div className="bg-blue-950/50 border border-blue-800 rounded-lg p-4">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="ml-auto text-xs text-blue-400 bg-blue-900/40 px-2 py-0.5 rounded-full border border-blue-800">
                            Waiting for vehicle...
                          </span>
                        </div>
                        <p className="text-slate-400 text-xs mb-3 leading-relaxed">
                          In the real system, an ultrasonic sensor automatically detects when a car enters the slot and changes the status to Occupied. Press the button below to simulate this.
                        </p>
                        <button
                          onClick={() => handleCarDetected(selectedSlot)}
                          className="w-full flex items-center justify-center gap-2 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold text-sm transition-colors"
                        >
                          <Car className="w-4 h-4" /> Simulate Car Detected
                        </button>
                      </div>
                    )}
                  </div>
